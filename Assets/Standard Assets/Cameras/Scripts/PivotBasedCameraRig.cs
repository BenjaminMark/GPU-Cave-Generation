	...
	virtual protected void Update()
	{
		...
		if (!Application.isPlaying && followTargetInEditMode)
		{
			...
			if (Mathf.Abs (cam.localPosition.x) > .5f || Mathf.Abs (cam.localPosition.y) > .5f)
			{
				...
				Debug.Log("[cam.localPosition.xy outside .5 detected] Camera Rig Warning: You should only adjust this Camera's Z position. The X and Y values must remain zero. Instead, move the Camera's parent (the \"Pivot\") to adjust the camera view");
