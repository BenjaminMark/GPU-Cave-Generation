...
public class ProtectCameraFromWallClip : MonoBehaviour
{
	...
	private float defaultDist;                         // the original distance to the camera before any modification are made
	...
	void Start() {
		...
		pivot = cam.parent;
		originalDist = defaultDist = cam.localPosition.magnitude;
	...
	public void tweakDefaultDistance(Vector3 dist, int sign){
		defaultDist += dist.magnitude*sign;
	}

	public void revertToOriginalDistance(){
		defaultDist = originalDist;
	}

	void LateUpdate() {

        // initially set the target distance
		float targetDist = defaultDist;
		...
		if (initialIntersect) {
			...
			hits = Physics.RaycastAll(ray, defaultDist - sphereCastRadius);
		} else {

            // if there was no collision do a sphere cast to see if there were any other collisions
			hits = Physics.SphereCastAll(ray, sphereCastRadius, defaultDist + sphereCastRadius);
		}
		...
		currentDist = Mathf.Clamp(currentDist,closestDistance,defaultDist);
