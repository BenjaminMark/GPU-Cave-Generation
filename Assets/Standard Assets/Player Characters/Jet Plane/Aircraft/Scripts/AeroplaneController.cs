...
{
	[SerializeField] private float jetCap = 0.25f;
	[SerializeField] private float speedCap = 6f;
	[SerializeField] private float gravityReduction = 0.5f;
	...
	public float AirBrakes { get; private set; }                         // Whether or not the air brakes are being applied.
	...
	public void Move(float rollInput, float pitchInput, float yawInput, float throttleInput, float airBrakes)//bool airBrakes)
    {
		...
		//rigidbody.velocity = velocityUsedToBe;
		...
		/*
		Vector3 change = rigidbody.velocity - aria * speedCap;
		change.y = rigidbody.velocity.y;

		//velocityUsedToBe = rigidbody.velocity;
		rigidbody.velocity = change;

		if(rigidbody.velocity.y<0)
			rigidbody.AddForce(-gravityReduction * Physics.gravity);
			*/
    }
	...
	void ClampInputs()
	{
		...
		/*
		mapping [A, B] to [1, airBrakesEffect]
		yourVal =  ((yourVal - A) / (B - A)) * (airBrakesEffect - 1) + 1;
		*/
		//AirBrakes = ((AirBrakes+1)/2)*(airBrakesEffect - 1) + 1;
		//Debug.Log("AirBrakes: "+AirBrakes);
		AirBrakes = Mathf.Clamp (ThrottleInput, -6, 6);
		AirBrakes = ((AirBrakes+6)/12)*(airBrakesEffect - 1) + 1;
	}
	...
	void CalculateDrag ()
	{
		...
		rigidbody.drag = (originalDrag + extraDrag) * airBrakesEffect / AirBrakes;
	...
	Vector3 aria = Vector3.zero;
	//Vector3 velocityUsedToBe = Vector3.zero;
	void CalculateLinearForces ()
	{
		...
		aria = EnginePower * transform.forward;
		...
		var liftPower = ForwardSpeed * ForwardSpeed * lift;// * aeroFactor;
		forces += liftPower * liftDirection -gravityReduction * Physics.gravity;

		aria.y = 0;
		//forces -= aria* speedCap;
		...
		rigidbody.velocity -= aria* jetCap;// + transform.forward * speedCap;
	...
	void CalculateTorque ()
	{
		...
		//torque += YawInput * yawEffect * transform.up;
		torque += YawInput * yawEffect * transform.up;
		torque += -YawInput * rollEffect * transform.forward;
		...
		//torque.y = rigidbody.angularVelocity.y;
