...
Shader "Custom/OutlinedDiffuseIce" {
	Properties {
...
//#include "/ScrawkShaderWorleyVoronoi/3D/ImprovedVoronoiNoise3D.cginc"
...
SubShader 
{
	...
	Pass {
		...
		ZTest LEqual
		Blend SrcAlpha OneMinusSrcAlpha //Off
		//ColorMask RGB //alpha channel not used
		//Lighting On
		//Fog { Mode Exp2 }
		...
		CGPROGRAM
			...
			half4 fragBase(v2f i) :COLOR { 
				...
				//n.x += (blended_bump_vec*_BumpPower);
				//n.x += (blended_bump_vec*-2.6);
				//n.z += (blended_bump_vec*_BumpPower);
				//n = normalize(n);
