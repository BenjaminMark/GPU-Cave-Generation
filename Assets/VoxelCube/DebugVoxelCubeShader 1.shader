Shader "Custom/particleCubeDebug" 
{
	...
	SubShader 
	{
		Pass 
		{
			...
			StructuredBuffer<PPParticle> postProcessBuffer;
			...
			FragInput vert (uint id : SV_VertexID, uint inst : SV_InstanceID)
			{
				...
				if(postProcessBuffer[inst].used > 1 &&
					postProcessBuffer[inst].used < 15){
					cb.r = 0;
					cb.g = 1;
					cb.b = 0;
					fragInput.color = cb;
				}else
				if(postProcessBuffer[inst].used==0){
				...
				if(postProcessBuffer[inst].used>=999999){
					...
					cb.b = 0;
					fragInput.color = cb;
					fragInput.position = mul (UNITY_MATRIX_MVP, float4(	postProcessBuffer[inst].position +
																	cubed_verts[id]*_generatedNoiseCubeSize
																	,
																	1));
				}
				
				if(postProcessBuffer[inst].used == 9){
					cb.r = 1;
					cb.g = 1;
					cb.b = 1;
					fragInput.color = cb;
